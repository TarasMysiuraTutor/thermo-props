...
export default function App() {
  ...
  const calculate = useCallback(() => {
    ...
    if (pRaw === null) {
      ...
      pMPa = Water.toMPa(pRaw, pressureUnit);
    ...
    const T = Water.toCelsius(raw, tempUnit);
    if (T < -100 || T > 800) {
      const range = tempUnit === 'C' ? '−100 – 800 °C'
                  : tempUnit === 'K' ? '173.15 – 1073.15 K'
                  : '−148 – 1472 °F';
    ...
    const next = {
      T, raw, pMPa, pRaw, pUnit: pressureUnit, tempUnit,
      token: Date.now(),
    };
    setResult(next);
    ...
    setHistory((prev) => {
      const filtered = prev.filter((e) => !(
        e.raw === raw && e.unit === tempUnit &&
        e.pRaw === pRaw && e.pUnit === pressureUnit
      ));
      const updated = [{ raw, unit: tempUnit, T, pRaw, pUnit: pressureUnit, pMPa, ts: Date.now() }, ...filtered];
      return updated.slice(0, HISTORY_LIMIT);
    });
  }, [tempValue, tempUnit, pressureValue, pressureUnit, t]);

  const onPickTemperatureC = useCallback((tC) => {
    let val = tC;
    ...
    setTimeout(() => {
      ...
      let pRaw = pRawStr === '' ? null : parseFloat(pRawStr);
      let pMPa;
      if (pRaw === null) { pMPa = Water.P_ATM_MPA; pRaw = pMPa; }
      else { pMPa = Water.toMPa(pRaw, pressureUnit); }
      if (T < -100 || T > 800) return;
      if (pMPa < 0.001 || pMPa > 100) return;
      setResult({ T, raw, pMPa, pRaw, pUnit: pressureUnit, tempUnit, token: Date.now() });
    }, 0);
  }, [tempUnit, pressureUnit, pressureValue]);
    ...
    <div className="page-wrap">
      ...
      <main className="main-content">
        <div className="container">
          ...
          {result && (
            <>
              <Results result={result} onPickTemperatureC={onPickTemperatureC} />
              <Chart result={result} />
